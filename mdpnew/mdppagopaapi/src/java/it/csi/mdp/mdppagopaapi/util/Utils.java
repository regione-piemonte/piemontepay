...
//	}
     ...
     * @param eventuale mappa di proprieta' per una configurazione specifica
     * @return L'istanza del proxy creato; il codice che richiama il metodo lo castera' all'interfaccia desiderata
     * @throws Exception
     */
    public static Object getProxyAPIService ( Class serviceInterfaceClass,
        ...
        try {
            logger.info ( "Inizio creazione client WS" );
            ...
            if ( proxy == null ) {
                logger.error ( "Reperimento del proxy fallito" );
                throw new UnrecoverableException ();
            }

            // impostazione custom dei valori soglia dei timeout
            logger.info ( "Endpoint di Servizio remoto: <" + address + ">" );
            ...
            logger.info ( "Proxy di Servizio remoto reperito con successo ..." );
            return proxy;
        } catch ( Exception e ) {
            logger.error ( "Reperimento del proxy fallito - causa: ", e );
            throw new UnrecoverableException ( e.getMessage (), e );
        } finally {
            logger.info ( "Fine creazione client WS" );
        }
    }
