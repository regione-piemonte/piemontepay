...
import it.csi.mdp.mdpetl.application.InvioDifferitoRPT;
import it.csi.mdp.mdpetl.application.ReinvioRT;
import it.csi.mdp.mdpetl.util.LogUtil;
import it.csi.mdp.mdpetl.util.StringUtils;

import java.util.concurrent.TimeUnit;

public class MainInvioDifferitoRPT {
	...
	private static void elaborate(String[] args, byte[] key) {
		...
		try {
			new InvioDifferitoRPT().elaborazioneReinvio(key);
		} catch (Exception e) {
			log.error("main", e);
			e.printStackTrace();
			System.exit(-1);
