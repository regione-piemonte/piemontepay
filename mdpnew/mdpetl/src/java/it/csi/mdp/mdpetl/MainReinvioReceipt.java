...
import it.csi.mdp.mdpetl.application.ReinvioReceipt;
...
public class MainReinvioReceipt {

	private static LogUtil log = new LogUtil ( MainReinvioRT.class );
	...
	private static void elaborateLocal () {
		...
		try {
			...
			new ReinvioReceipt ().elaborazioneReinvio ( key );
		} catch ( Exception e ) {
			log.error ( "main", e );
			e.printStackTrace ();
			System.exit ( -1 );
