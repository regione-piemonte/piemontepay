...
import it.csi.mdp.mdpetl.application.InoltroFlussiRendicontazione;
import it.csi.mdp.mdpetl.util.LogUtil;
import it.csi.mdp.mdpetl.util.StringUtils;

import java.util.concurrent.TimeUnit;

public class MainRiconciliazioneFlussi {
	...
	private static void elaborate(String[] args, byte[] key) {
		...
		try {
			new InoltroFlussiRendicontazione().inoltraFlussi(args[0], key);
		} catch (Exception e) {
			log.error("main", e);
			e.printStackTrace();
			System.exit(-1);
