...
<s:form id="cpDettaglioGatewayNewClone" action="cpDettaglioGatewayNewClone" method="post">
...
<s:if test="isWidgetVisible('cpDettaglioGatewayNewClone','mnuView')" >
...
<div class="tabSetPanel" id="tstGateway">
	<ul>
		<s:url id="cpDettaglioGatewayNewClone_tstGateway_pDatiGeneraliGateway_URL" action="cpDettaglioGatewayNewClone" method="handleChangeTab" includeParams="get">
			<s:param name="selectedTabKey" value="%{'cpDettaglioGatewayNewClone_tstGateway_selectedMultiPanel'}" />
			<s:param name="selectedTabValue" value="%{'cpDettaglioGatewayNewClone_tstGateway_pDatiGeneraliGateway'}" />
		</s:url>
		<s:if test="%{#session.cpDettaglioGatewayNewClone_tstGateway_selectedMultiPanel == 'cpDettaglioGatewayNewClone_tstGateway_pDatiGeneraliGateway' || #session.cpDettaglioGatewayNewClone_tstGateway_selectedMultiPanel == null}">
			<li class="item-1 active"><span class="activeItem"><s:text name="cpDettaglioGatewayNewClone.pDatiGeneraliGateway.label" /></span></li>
		</s:if>
		<s:else>
			<li class="item-1"><s:a href="%{cpDettaglioGatewayNewClone_tstGateway_pDatiGeneraliGateway_URL}"><s:text name="cpDettaglioGatewayNewClone.pDatiGeneraliGateway.label" /></s:a></li>
		</s:else>
	</ul>
</div>
...
<s:if test="#session.cpDettaglioGatewayNewClone_tstGateway_selectedMultiPanel == 'cpDettaglioGatewayNewClone_tstGateway_pDatiGeneraliGateway' || #session.cpDettaglioGatewayNewClone_tstGateway_selectedMultiPanel == null">
	<s:include value="/jsp/tabs/cpDettaglioGatewayNewClone_tstGateway_pDatiGeneraliGateway.jsp" ></s:include>
</s:if>
