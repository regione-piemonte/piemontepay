...
/**
 * CpDettaglioGatewayNewCloneAction Action Class.
 *
 * @author GuiGen
 */
@Validation()
@Conversion()
public class CpDettaglioGatewayNewCloneAction extends BaseAction
		implements
			Preparable {
	...
	public Class modelClass() {
		return it.csi.mdp.mdpboweb.dto.mdpbackoffice.CpDettaglioGatewayNewCloneModel.class;
	}
	...
	/**
	 * Gestione dell'evento CLICKED sul widget [btGatewaySel]
	 */
	@MethodProtection(level = "REJECT_SAME")
	public String handleBtGatewaySel_CLICKED() throws CommandExecutionException {
		ICommand action = initCommand("btGatewaySel", "CLICKED");
		if (log.isDebugEnabled()) {
			log.debug("[CpDettaglioGatewayNewCloneAction::handleBtGatewaySel_CLICKED] dump model before");
			dumpModel(false);
		}
		...
		if (result != null) {
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtGatewaySel_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtGatewaySel_CLICKED] returning result ["
						+ result + "]");
			}
			return result;
			...
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtGatewaySel_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtGatewaySel_CLICKED] returning default result [SUCCESS]");
			}
			return SUCCESS;
	...
	public String handleBtnEliminaAttributo_CLICKED()
		...
		if (log.isDebugEnabled()) {
			log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnEliminaAttributo_CLICKED] dump model before");
			dumpModel(false);
		}
		...
		if (result != null) {
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnEliminaAttributo_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnEliminaAttributo_CLICKED] returning result ["
						+ result + "]");
			}
			return result;
			...
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnEliminaAttributo_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnEliminaAttributo_CLICKED] returning default result [SUCCESS]");
			}
			return SUCCESS;
	...
	public String handleBtnAggiungiAttributo_CLICKED()
		...
		if (log.isDebugEnabled()) {
			log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnAggiungiAttributo_CLICKED] dump model before");
			dumpModel(false);
		}
		...
		if (result != null) {
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnAggiungiAttributo_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnAggiungiAttributo_CLICKED] returning result ["
						+ result + "]");
			}
			return result;
			...
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnAggiungiAttributo_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnAggiungiAttributo_CLICKED] returning default result [SUCCESS]");
			}
			return SUCCESS;
	...
	public String handleBtnIndietro_CLICKED() throws CommandExecutionException {
		...
		if (log.isDebugEnabled()) {
			log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnIndietro_CLICKED] dump model before");
			dumpModel(false);
		}
		...
		if (result != null) {
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnIndietro_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnIndietro_CLICKED] returning result ["
						+ result + "]");
			}
			return result;
			...
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnIndietro_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnIndietro_CLICKED] returning default result [SUCCESS]");
			}
			return SUCCESS;
	...
	public String handleBtnSalva_CLICKED() throws CommandExecutionException {
		...
		if (log.isDebugEnabled()) {
			log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnSalva_CLICKED] dump model before");
			dumpModel(false);
		}
		...
		if (result != null) {
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnSalva_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnSalva_CLICKED] returning result ["
						+ result + "]");
			}
			return result;
			...
			if (log.isDebugEnabled()) {
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnSalva_CLICKED] dump model after");
				dumpModel(false);
				log.debug("[CpDettaglioGatewayNewCloneAction::handleBtnSalva_CLICKED] returning default result [SUCCESS]");
			}
			return SUCCESS;
	...
	public String provideTbListaAttributi_DATASET()
		...
		if (isTableClearStatus("cpDettaglioGatewayNewClone_tbListaAttributi")) {
	...
	public String provideTLingue_DATASET() throws CommandExecutionException {
		...
		if (isTableClearStatus("cpDettaglioGatewayNewClone_tLingue")) {
			tableStatus.setClearStatus(true);
		}
		TableDataArranger arranger = new TableDataArranger();
		if (dataToProvide != null) {
	...
	public String provideTDivise_DATASET() throws CommandExecutionException {
		...
		if (isTableClearStatus("cpDettaglioGatewayNewClone_tDivise")) {
			tableStatus.setClearStatus(true);
		}
		TableDataArranger arranger = new TableDataArranger();
		if (dataToProvide != null) {
	...
	public void validate() {
		/*PROTECTED REGION ID(R-365051684) ENABLED START*/
	...
	public boolean isWidgetDisabled(String cpName, String widgShortName) {
		...
		if (ctr != null) {
			try {
				...
				log.error("[CpDettaglioGatewayNewCloneAction::isWidgetDisabled] errore durante verifica->disable");
	...
	public boolean isWidgetVisible(String cpName, String widgShortName) {
		...
		if (ctr != null) {
			try {
				...
				log.error("[CpDettaglioGatewayNewCloneAction::isWidgetVisible] errore durante verifica->hide");
	...
	private ICommand initOnRefreshCommand() {
		// ExecCommand begin
		String[] resultNames4refreshPanel = new String[]{"NUOVO", "SIEXTRA",
				"NOEXTRA"};
		...
		String[] act_onRefresh_resnuovoActionstep_0_on = new String[]{
				...
				"btnSalva", "tbListaAttributi", "tLingue", "tDivise",
				"btnAggiungiAttributo", "txtNome", "txtDescrizione",
				"btnEliminaAttributo"};

		String[] act_onRefresh_resnuovoActionstep_0_off = new String[]{
				"cbGateways", "btGatewaySel"};
		...
		ScreenStateCommand act_onRefresh_resnuovoActionstep_0 = new ScreenStateCommand(
				"cpDettaglioGatewayNewClone", "INIZIALE",
		...
		// Screen State Command begin
		//prepara i nomi dei componenti target da abilitare/disabilitare
		String[] act_onRefresh_ressiextraAction_1_on = new String[]{"mnuView",
				"tfIdGateway", "tfDescrGateway", "tfProviderGateway",
				"tfServiceJNDI", "tbListaAttributi", "btnEliminaAttributo",
				"txtNome", "txtDescrizione", "btnAggiungiAttributo", "tLingue",
				"tDivise", "btnIndietro", "btnSalva"};

		String[] act_onRefresh_ressiextraAction_1_off = new String[]{
				"cbGateways", "btGatewaySel"};

		String[] act_onRefresh_ressiextraAction_1_show = new String[]{
				"mnuView", "tfIdGateway", "tfDescrGateway",
				"tfProviderGateway", "tfServiceJNDI", "tbListaAttributi",
				"btnEliminaAttributo", "txtNome", "txtDescrizione",
				"btnAggiungiAttributo", "tLingue", "tDivise", "btnIndietro",
				"btnSalva"};

		String[] act_onRefresh_ressiextraAction_1_hide = new String[]{
				"cbGateways", "btGatewaySel"};

		ScreenStateCommand act_onRefresh_ressiextraAction_1 = new ScreenStateCommand(
				"cpDettaglioGatewayNewClone", "SIEXTRA",
				act_onRefresh_ressiextraAction_1_on,
				act_onRefresh_ressiextraAction_1_off,
				act_onRefresh_ressiextraAction_1_show,
				act_onRefresh_ressiextraAction_1_hide);
		//Screen State Command end
		actionsForResults4refreshPanel[1] = act_onRefresh_ressiextraAction_1;
		// Screen State Command begin
		//prepara i nomi dei componenti target da abilitare/disabilitare
		String[] act_onRefresh_resnoextraAction_2_on = new String[]{"mnuView",
				"tfIdGateway", "tfDescrGateway", "tfProviderGateway",
				"tfServiceJNDI", "txtNome", "txtDescrizione",
				"btnAggiungiAttributo", "tLingue", "tDivise", "btnIndietro",
				"btnSalva"};

		String[] act_onRefresh_resnoextraAction_2_off = new String[]{
				"cbGateways", "btGatewaySel", "tbListaAttributi",
				"btnEliminaAttributo"};

		String[] act_onRefresh_resnoextraAction_2_show = new String[]{
				"mnuView", "tfIdGateway", "tfDescrGateway",
				"tfProviderGateway", "tfServiceJNDI", "txtNome",
				"txtDescrizione", "btnAggiungiAttributo", "tLingue", "tDivise",
				"btnIndietro", "btnSalva"};

		String[] act_onRefresh_resnoextraAction_2_hide = new String[]{
				"cbGateways", "btGatewaySel", "tbListaAttributi",
				"btnEliminaAttributo"};

		ScreenStateCommand act_onRefresh_resnoextraAction_2 = new ScreenStateCommand(
				"cpDettaglioGatewayNewClone", "NOEXTRA",
				act_onRefresh_resnoextraAction_2_on,
				act_onRefresh_resnoextraAction_2_off,
				act_onRefresh_resnoextraAction_2_show,
				act_onRefresh_resnoextraAction_2_hide);
		//Screen State Command end
		actionsForResults4refreshPanel[2] = act_onRefresh_resnoextraAction_2;
		...
		ExecCommand act_onRefresh_1 = new ExecCommand(resultNames4refreshPanel,
			...
			public ExecResults doLogic(Object theModel)
				...
				try {
					...
					watcher.stop();
					watcher.dumpElapsed("CpDettaglioGatewayNewCloneAction",
	...
	protected Map<String, UISecConstraint> getPageVisibilityUIConstraints() {
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "mnuView",
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "cbGateways",
				UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR, true, true);
		...
		// constraint fittizio per btGatewaySel
		UISecConstraint btGatewaySel_defaultVisib_ctr =

		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "btGatewaySel",
				UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR, true, true);

		UISecConstraint[] btGatewaySel_constraints = new UISecConstraint[]{btGatewaySel_defaultVisib_ctr};
		UISecConstraint btGatewaySel_ctr = new ComplexUISecConstraint(
				btGatewaySel_constraints);
		allConstraints.put("btGatewaySel", btGatewaySel_ctr);

		// constraint fittizio per tfIdGateway
		UISecConstraint tfIdGateway_defaultVisib_ctr =

		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "tfIdGateway",
				UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR, true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone",
				"tfDescrGateway", UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR,
				true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "tfServiceJNDI",
				UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR, true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone",
				"tbListaAttributi", UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR,
				true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "txtNome",
				UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR, true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone",
				"txtDescrizione", UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR,
				true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "tLingue",
				UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR, true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "tDivise",
				UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR, true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "btnIndietro",
				UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR, true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "btnSalva",
				UISecConstraint.VISIB_CONSTRAINED_BEHAVIOR, true, true);
	...
	protected Map<String, UISecConstraint> getPageONOFFUIConstraints() {
		...
		// constraint fittizio per btGatewaySel
		UISecConstraint btGatewaySel_defaultOnoff_ctr =

		new DummyUISecConstraint("cpDettaglioGatewayNewClone", "btGatewaySel",
				UISecConstraint.ONOFF_CONSTRAINED_BEHAVIOR, true, true);

		UISecConstraint[] btGatewaySel_constraints = new UISecConstraint[]{btGatewaySel_defaultOnoff_ctr};
		UISecConstraint btGatewaySel_ctr = new ComplexUISecConstraint(
				btGatewaySel_constraints);
		allConstraints.put("btGatewaySel", btGatewaySel_ctr);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone",
				"tfDescrGateway", UISecConstraint.ONOFF_CONSTRAINED_BEHAVIOR,
				true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone",
				"tbListaAttributi", UISecConstraint.ONOFF_CONSTRAINED_BEHAVIOR,
				true, true);
		...
		new DummyUISecConstraint("cpDettaglioGatewayNewClone",
				"txtDescrizione", UISecConstraint.ONOFF_CONSTRAINED_BEHAVIOR,
				true, true);
	...
	void dumpModel(boolean pre) {
		log.debug("[CpDettaglioGatewayNewCloneAction::dumpmodel] START");

		log.debug("[CpDettaglioGatewayNewCloneAction::dumpmodel] #### DUMP del model della action "
				+ this.getClass()
				+ (pre ? " prima dell'azione" : " dopo l'azione"));
		log.debug("[CpDettaglioGatewayNewCloneAction::dumpmodel] [a] campi pubblici del model");
		try {
			...
			for (int i = 0; i < props.length; i++) {
				...
				if (m != null) {
					...
					log.debug("[CpDettaglioGatewayNewCloneAction::dumpmodel] "
							+ pd.getName() + ":" + pval);
				}
			}
		} catch (IllegalAccessException e) {
			log.error("[CpDettaglioGatewayNewCloneAction::dumpmodel] Errore nel dump"
					+ e + ", ignoro");
		...
		log.debug("[CpDettaglioGatewayNewCloneAction::dumpmodel] [b] stato dei widget");
		Object cpWidgetStatus = session.get("cpDettaglioGatewayNewClone");
		log.debug("[CpDettaglioGatewayNewCloneAction::dumpmodel] "
				+ cpWidgetStatus);
		log.debug("[CpDettaglioGatewayNewCloneAction::dumpmodel] [c] sessione");
		log.debug("[CpDettaglioGatewayNewCloneAction::dumpmodel] " + session);
	}
	...
	private ICommand initCommand(String sourceWidget, String eventType) {
		...
		// contiene i comandi del widget btGatewaySel per ogni Ev.H.
		HashMap<String, ICommand> cmds4btGatewaySelByEvh = new HashMap<String, ICommand>();

		cmds4btGatewaySelByEvh
				.put("CLICKED", initCommandBtGatewaySel_CLICKED());
		cmdsByWidget.put("btGatewaySel", cmds4btGatewaySelByEvh);
	...
	private ICommand initCommandBtGatewaySel_CLICKED() {
		// ExecCommand begin
		String[] resultNames4valorizzaDettGateway = new String[]{"POST_SELEZIONE"};

		ICommand[] actionsForResults4valorizzaDettGateway = new ICommand[1];
		// SequenceCommand begin
		// Screen State Command begin
		//prepara i nomi dei componenti target da abilitare/disabilitare
		String[] act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0_on = new String[]{};

		String[] act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0_off = new String[]{};

		String[] act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0_show = new String[]{};

		String[] act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0_hide = new String[]{};

		ScreenStateCommand act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0 = new ScreenStateCommand(
				"cpDettaglioGatewayNewClone",
				"??",
				act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0_on,
				act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0_off,
				act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0_show,
				act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0_hide);
		//Screen State Command end

		ICommand[] act_actions_clicked_btGatewaySel_respost_selezioneAction_0_steps = new ICommand[]{act_actions_clicked_btGatewaySel_respost_selezioneActionstep_0};
		SequenceCommand act_actions_clicked_btGatewaySel_respost_selezioneAction_0 = new SequenceCommand(
				act_actions_clicked_btGatewaySel_respost_selezioneAction_0_steps);
		// SequenceCommand end
		actionsForResults4valorizzaDettGateway[0] = act_actions_clicked_btGatewaySel_respost_selezioneAction_0;

		ExecCommand act_actions_clicked_btGatewaySel_1 = new ExecCommand(
				resultNames4valorizzaDettGateway,
				actionsForResults4valorizzaDettGateway) {
			public ExecResults doLogic(Object theModel)
					throws CommandExecutionException {
				it.csi.util.performance.StopWatch watcher = new it.csi.util.performance.StopWatch(
						it.csi.mdp.mdpboweb.util.Constants.APPLICATION_CODE);
				...
				try {
					...
					throw new CommandExecutionException(
							"errore non gestito nell'esecuzione del metodo [valorizzaDettGateway]:"
									+ e.getMessage(), e);
				}
		...
		// Exec Action end
		return act_actions_clicked_btGatewaySel_1;
	}
	...
	private ICommand initCommandBtnSalva_CLICKED() {
		...
		String[] act_actions_clicked_btnSalva_resokActionstep_0_appdataToBeRemovedFromSession = new String[]{
				"isPostBack", "associazioniGW_MP", "associazioneGW_MP",
