...
public class CsiUdNumericDecValidator extends DoubleRangeFieldValidator {
	...
	public void validate(Object object) throws ValidationException {
		...
		if ((maxInclusiveValue != null && value.compareTo(maxInclusiveValue) > 0)
				|| (minInclusiveValue != null && value
						.compareTo(minInclusiveValue) < 0)
				|| (maxExclusiveValue != null && value
						.compareTo(maxExclusiveValue) >= 0)
				|| (minExclusiveValue != null && value
						.compareTo(minExclusiveValue) <= 0)) {
	...
	private Double parseDouble(String value) {
		if (value != null) {
			try {
				...
				log.warn("[CsiUdNumericDecValidator::parseDouble]: Unable to parse given double parameter "
						+ value);
			}
		}
		return null;
