...
public class PaymentBean implements SessionBean
{
	...
	CreateException, MissingParameterException, IllegalAccessException, InvocationTargetException
	{
		...
		log.debug("[PaymentBean::getModalitaPagamento] END");
		return (AppGatewayInformativa[])listAppGwInformativa.toArray(new AppGatewayInformativa[0]);
	}
	...
	public String startTransazione(Transazione transaction, TransazioneExtraAttribute[] tea, List<RPTData> elencoRPT) throws RemoteException, DaoException,
			NamingException, CreateException, MissingParameterException
	{
		...
		try {
			...
			urlToCall = cPayment.startTransazione(transaction, tea, elencoRPT);
		} catch (Exception e) {
			LoggerUtil.error("ERRORE GENERICO", e);
			throw new DaoException(e.getMessage(), e);
		}
	...
	MissingParameterException {
		this.setStatoTransazione(transactionId, stato, gwStato, null);
	}
	public void setStatoTransazione(String transactionId, long stato, String gwStato, String iuv) throws RemoteException, DaoException,
			MissingParameterException
	{
		...
		try
		{
			...
			cPayment.setStatoTransazione(transactionId, stato, gwStato, iuv);
		} catch (Exception e)
		{
			e.printStackTrace();
...
/*	public boolean testResources() throws RemoteException, it.csi.csi.wrapper.SystemException
   ...
   */
	...
	public String startTransazione(Transazione t, TransazioneExtraAttribute [] tea) throws RemoteException, DaoException, NamingException, CreateException, MissingParameterException {
		...
		StopWatch stopWatch = new StopWatch(Constants.APPLICATION_CODE);
		stopWatch.start();
		String url = startTransazione(t, tea, null);
		stopWatch.stop();
		stopWatch.dumpElapsed("PaymentBean", "startTransazione()", "chiamata startTransazione", "startTransazione");
		LoggerUtil.end();
		return url;
