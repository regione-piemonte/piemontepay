	...
	public ApplicationDetail mapRow(ResultSet rs, int row) throws SQLException
	{
		...
		if (this.sKey != null)
		{
			...
			try
			{
				...
				{
					byte[] encrypted = rs.getString(5).getBytes();
					encrypted = Base64.decode(new String(encrypted));
					original = cipher.doFinal(encrypted);
					dto.setMerchantidpassword(new String(original));
				}
				if (rs.getString(18) != null)
				{
					byte[] encrypted = rs.getString(18).getBytes();
					encrypted = Base64.decode(new String(encrypted));
					original = cipher.doFinal(encrypted);
					dto.setMacAvvio(new String(original));
				}
				if (rs.getString(19) != null)
				{
					byte[] encrypted = rs.getString(19).getBytes();
					encrypted = Base64.decode(new String(encrypted));
					original = cipher.doFinal(encrypted);
					dto.setMacEsito(new String(original));
				}

			} catch (Exception e)
			{
				// TODO Auto-generated catch block
				log.error("errore in decriptazione mac dto:" + dto.toString());
			}
		}
		return dto;
	}
