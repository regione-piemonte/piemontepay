...
@Transactional
public class GetOpzioniAssociazioneUtenteTematicaOperationTest extends GetOpzioniParentOperationTest<GetOpzioniAssociazioneUtenteTematicaOutput> {
    ...
    public void shouldNotReturnWithFlagAnnullato () {
        ...
        GetOpzioniInput input = getInput ();
        GetOpzioniAssociazioneUtenteTematicaOutput output = call ( input );

        assertEquals ( Constants.RESULT_CODES.OK, output.getCodiceEsito () );

        int size0 = 0;
        for ( GetOpzioniAssociazioneUtenteTematicaTematicaOutputDto o: output.getTematiche () ) {
            if ( o.getCodiciVersamento () != null ) {
                size0 += o.getCodiciVersamento ().size ();
            }
        }

        assertTrue ( size0 > 0 );

        List<CodiceVersamento> all = codiceVersamentoRepository.findAll ();
        assertTrue ( all.size () >= 3 );
        for ( int i = 0; i <= all.size () / 2; i++ ) {
            CodiceVersamento entity = all.get ( i );
            entity.setFlagAnnullato ( true );
            codiceVersamentoRepository.save ( entity );
        }
        ...
        for ( GetOpzioniAssociazioneUtenteTematicaTematicaOutputDto o: output.getTematiche () ) {
            if ( o.getCodiciVersamento () != null ) {
                size1 += o.getCodiciVersamento ().size ();
            }
        }
