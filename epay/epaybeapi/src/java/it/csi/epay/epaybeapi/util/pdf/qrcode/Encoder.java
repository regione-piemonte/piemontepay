...
package it.csi.epay.epaybeapi.util.pdf.qrcode;
...
public final class Encoder {
	 ...
	 */
	static void terminateBits(int numDataBytes, BitVector bits) throws
					WriterException {
	...
	static void interleaveWithECBytes( BitVector bits, int numTotalBytes,
					int numDataBytes, int numRSBlocks, BitVector result) throws
					WriterException {
	...
	static void appendAlphanumericBytes(String content, BitVector bits) throws
					WriterException {
		int length = content.length();
		int i = 0;
		while (i < length) {
	...
	static void appendKanjiBytes(String content, BitVector bits) throws
					WriterException {
		byte[] bytes;
		try {
			bytes = content.getBytes("Shift_JIS");
