...
/**
 * Spring mapper test for "EpayDAutorizzazioneChiamante" <br>
 *
 * @author EII
 */
...
@Transactional
public class EpayDAutorizzazioneChiamanteMapperTest extends ParentHttpCallTest {

    @Autowired
    private EpayDAutorizzazioneChiamanteRepository repository;

    @Autowired
    private EpayDAutorizzazioneChiamanteMapper mapper;

    @Test
    public void mappingShouldWork () {
        Page<EpayDAutorizzazioneChiamante> records = repository.findAll( new PageRequest ( 0, 10 ) );
        assertNotNull( records );
        
        for ( EpayDAutorizzazioneChiamante record : records ) {
        	EpayDAutorizzazioneChiamanteDTO dto = mapper.toDTO( record );
        	assertNotNull( dto );
        	
        }
		
		List<EpayDAutorizzazioneChiamanteDTO> dtos = mapper.toDTO( records );
		assertNotNull( dtos );
