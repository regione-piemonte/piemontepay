	...
	public PdfLine processLine ( float leftX, float width, int alignment, int runDirection, int arabicOptions, float minY, float yLine, float descender ) {
		...
		for ( ; currentChar < totalTextLength; ++currentChar ) {
			...
			if ( ck.isTab () ) {
				...
				if ( width - charWidth < 0 )
					break;
				if ( tabStop != null && tabStop.getAlignment () == TabStop.Alignment.ANCHOR && Float.isNaN (
								tabStopAnchorPosition ) && tabStop.getAnchorChar () == (char) uniC ) {
		...
		if ( newCurrentChar == currentChar - 1 ) { // middle of word
			...
			if ( he != null ) {
				...
				if ( word != null ) {
					...
					String pre = he.getHyphenatedWordPre ( new String ( text, word[0], word[1] - word[0] ), lastValidChunk.font ().getFont (),
									lastValidChunk.font ().size (), testWidth );
					String post = he.getHyphenatedWordPost ();
					if ( !pre.isEmpty () ) {
						PdfChunk extra = new PdfChunk ( pre, lastValidChunk );
						currentChar = word[1] - post.length ();
						return new PdfLine ( 0, originalWidth, testWidth - lastValidChunk.width ( pre ), alignment, false,
		...
		for ( ; startIdx <= lastIdx; ++startIdx ) {
			...
			if ( detailChunks[startIdx].isTab ()
				...
				if ( PdfChunk.noPrint ( ck.getUnicodeEquivalent ( c ) ) )
					continue;
				if ( tabStop != null && tabStop.getAlignment () != TabStop.Alignment.ANCHOR && Float.isNaN (
								tabStopAnchorPosition ) && tabStop.getAnchorChar () == (char) ck.getUnicodeEquivalent ( c ) ) {
