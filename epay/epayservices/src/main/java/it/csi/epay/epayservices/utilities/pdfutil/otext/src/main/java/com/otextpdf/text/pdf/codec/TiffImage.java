...
import it.csi.epay.epayservices.utilities.pdfutil.otext.src.main.java.com.otextpdf.text.Jpeg;
import it.csi.epay.epayservices.utilities.pdfutil.otext.src.main.java.com.otextpdf.text.error_messages.MessageLocalization;
import it.csi.epay.epayservices.utilities.pdfutil.otext.src.main.java.com.otextpdf.text.exceptions.InvalidImageException;
...
public class TiffImage {
		...
		try {
			...
			if ( direct && rowsStrip == h ) { //single strip, direct
				...
				for ( int k = 0; k < offset.length; ++k ) {
					...
					TIFFFaxDecoder decoder = new TIFFFaxDecoder ( fillOrder, w );
	...
	protected static Image getTiffImageColor ( TIFFDirectory dir, RandomAccessFileOrArray s ) {
		try {
			...
			long[] offset = getArrayLongShort ( dir, TIFFConstants.TIFFTAG_STRIPOFFSETS );
			long[] size = getArrayLongShort ( dir, TIFFConstants.TIFFTAG_STRIPBYTECOUNTS );
			if ( ( size == null || ( size.length == 1 && ( size[0] == 0 || size[0] + Objects.requireNonNull ( offset )[0] > s.length () ) ) ) && h == rowsStrip ) { // some TIFF producers are really lousy, so...
				assert offset != null;
				size = new long[] { s.length () - (int) offset[0] };
			}
				...
				if ( predictorField != null ) {
					...
					if ( predictor == 2 && bitsPerSample != 8 ) {
						throw new RuntimeException (
										MessageLocalization.getComposedMessage ( "1.bit.samples.are.not.supported.for.horizontal.differencing.predictor",
														bitsPerSample ) );
					}
				}
			}
			...
			if ( compression == TIFFConstants.COMPRESSION_OJPEG ) {
				...
				if ( size.length > 1 )
					throw new IOException (
									MessageLocalization.getComposedMessage ( "compression.jpeg.is.only.supported.with.a.single.strip.this.image.has.1.strips",
													size.length ) );
	...
	static void ProcessExtraSamples ( DeflaterOutputStream zip, DeflaterOutputStream mzip, byte[] outBuf, int samplePerPixel, int bitsPerSample, int width,
