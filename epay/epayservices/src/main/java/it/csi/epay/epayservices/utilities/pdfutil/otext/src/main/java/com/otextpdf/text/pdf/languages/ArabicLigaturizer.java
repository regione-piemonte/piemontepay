...
import it.csi.epay.epayservices.utilities.pdfutil.otext.src.main.java.com.otextpdf.text.pdf.BidiLine;
import it.csi.epay.epayservices.utilities.pdfutil.otext.src.main.java.com.otextpdf.text.pdf.BidiOrder;
...
public class ArabicLigaturizer implements LanguageProcessor {
	...
	public static int arabic_shape ( char[] src, int srcoffset, int srclength, char[] dest, int destoffset, int level ) {
		char[] str = new char[srclength];
		System.arraycopy ( src, srcoffset, str, 0, srclength + srcoffset - srcoffset );
	...
	public static void processNumbers ( char[] text, int offset, int length, int options ) {
		...
		if ( ( options & DIGITS_MASK ) != 0 ) {
			...
			case DIGITS_EN2AN_INIT_LR:
				shapeToArabicDigitsWithContext ( text, length, digitBase, false );
				break;

			case DIGITS_EN2AN_INIT_AL:
				shapeToArabicDigitsWithContext ( text, length, digitBase, true );
				break;
	...
	static void shapeToArabicDigitsWithContext ( char[] dest, int length, char digitBase, boolean lastStrongWasAL ) {
