...
import it.csi.epay.epayapi.integration.repository.EpayTTipoPagamentoRepository;
import it.csi.epay.epayapi.testbed.config.UnitTestDB;
import it.csi.epay.epayapi.testbed.model.ParentHttpCallTest;

/**
 * Spring mapper test for "EpayTTipoPagamento" <br>
 *
...
@Transactional
public class EpayTTipoPagamentoMapperTest extends ParentHttpCallTest {

    @Autowired
    private EpayTTipoPagamentoRepository repository;

    @Autowired
    private EpayTTipoPagamentoMapper mapper;

    @Test
    public void mappingShouldWork () {
        Page<EpayTTipoPagamento> records = repository.findAll( new PageRequest ( 0, 10 ) );
        assertNotNull( records );
        
        for ( EpayTTipoPagamento record : records ) {
        	EpayTTipoPagamentoDTO dto = mapper.toDTO( record );
        	assertNotNull( dto );
        	
        }
		
		List<EpayTTipoPagamentoDTO> dtos = mapper.toDTO( records );
		assertNotNull( dtos );
