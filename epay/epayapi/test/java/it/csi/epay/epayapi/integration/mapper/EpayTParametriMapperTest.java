...
/**
 * Spring mapper test for "EpayTParametri" <br>
 *
...
@Transactional
public class EpayTParametriMapperTest extends ParentHttpCallTest {

    @Autowired
    private EpayTParametriRepository repository;

    @Autowired
    private EpayTParametriMapper mapper;

    @Test
    public void mappingShouldWork () {
        Page<EpayTParametri> records = repository.findAll( new PageRequest ( 0, 10 ) );
        assertNotNull( records );
        
        for ( EpayTParametri record : records ) {
        	EpayTParametriDTO dto = mapper.toDTO( record );
        	assertNotNull( dto );
        	
        }
		
		List<EpayTParametriDTO> dtos = mapper.toDTO( records );
		assertNotNull( dtos );
