...
public class ConfigurazioneService {
	...
	public ConfigurazioneServiceElementsDTO getMaxElementPerPage ( String serviceName, User user, String organizationFiscalCode, long initialMoment ) {
		var configurazioneEntity = configurazioneRepository.findByCodice ( CONFIG_MAX_ELEMENTS_PER_PAGE );
		if ( configurazioneEntity == null ) {
			var response = generateBusinessErrorResponse ( serviceName, ERROR_VALORE_DI_CONFIGURAZIONE_NOT_DEFINED, CONFIG_MAX_ELEMENTS_PER_PAGE );
		...
		var elements = Integer.parseInt ( configurazioneEntity.getValore () );
		Log.infof ( "Impostato elements al valore di default:%d", elements );
