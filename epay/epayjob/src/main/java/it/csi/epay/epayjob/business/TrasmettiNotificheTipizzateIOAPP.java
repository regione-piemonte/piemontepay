...
import it.csi.epaypa.notificheIoApp.NotificaTipizzataIoAppBuildMessage;


public class TrasmettiNotificheTipizzateIOAPP extends RtXml {
	...
	private String url; 
    public TrasmettiNotificheTipizzateIOAPP ( JobContext context, JobFacade jobFacade, ConfigurazioneFacade configurazioneFacade ) throws IllegalArgumentException, NoDataException {
		super();
...
//                log.info(methodName, "url: "+url);
            ...
            System.out.println ( methodName+" - errore lettura parametri. "+t );
        } finally {
...
//        List<Long> idPagamentiEnte = new LinkedList<Long>();
        ...
        NotificaTipizzataIoAppBuildMessage builderMessage = new NotificaTipizzataIoAppBuildMessage();
        
//       
        try {
//        //richiama epayservices per aver enti con notifiche da inviare e passphrase ente da passare a Notify
        enti = jobFacade.ricercaEntiNotify(); // fare un metodo che mocki il servizio
        ...
        System.out.println ( methodName+" - numero enti:  "+(null!= enti? enti.size():0 ));
    		...
    		context.info(methodName, "Nessun ente trovato abilitato all'invio: "+nde);
    		 System.out.println ( methodName+" - Nessun ente trovato abilitato all'invio:  "+nde);
    		return;
		}
        catch (Exception e) {
    		context.info(methodName, "Non e' stato possibile estrarre gli enti: "+e);
    		 System.out.println ( methodName+" - Non e' stato possibile estrarre gli enti:  "+e );
		}
...
//        //cliclo sugli enti e chiedo i pagamenti per ogni ente No Paginazione
        for (EntePassphrase ente : enti) {
            ...
            try
            {
                tipiPagamentoEnte = jobFacade.ricercaTipiPagamentoNotifyPerEnte ( ente.getIdEnte ()); 
            }
            catch (NoDataException nde) {
                context.info(methodName, "Nessun tipo pagamento trovato abilitato all'invio: "+nde);
                System.out.println ( methodName+" - Nessun tipo pagamento trovato abilitato all'invio: "+nde );

            }
            catch (Exception e) {
                context.info(methodName, "Non e' stato possibile estrarre tipi pagamento per ente: "+e);
                System.out.println ( methodName+" - \"Non e' stato possibile estrarre tipi pagamento per ente "+e );
            }
...
//                    TODO controllare che la descrizione  sia della lunghezza appropriata
                    try {
                        List<Pagamento> pagamentiEnte = jobFacade.ricercaPagamentiValidiPerTipoPagamento(tipoPagamento.getIdTipoPagamento ());
                        ...
                        if (null!= pagamentiEnte && !pagamentiEnte.isEmpty())
                        {
                            for (Pagamento pagamento : pagamentiEnte) {
                                ...
                                if (pagamento.getTipoPagamento ().getDescrizioneTextCov ().length ()<=maxNumDesc)
                                {
                                    ...
                                    System.out.println ( methodName+" - message:  "+msg.toString());
                        ...
                        System.out.println ( methodName+" - nessun tipo pagamento valido trovato  per invio notifica tipizzata per tipo pagamento:  "+tipoPagamento.getIdTipoPagamento ()+" - " +nde );
                        ...
                        System.out.println ( methodName+" - Errore in fase di reperimento del messaggio per tipo pagamento:  "+tipoPagamento.getIdTipoPagamento ()+ ": "+ ie );
                    }
                    catch (Exception e) {
                        ...
                        System.out.println ( methodName+" -  - Errore in fase di reperimento del messaggio per tipo pagamento:  "+tipoPagamento.getIdTipoPagamento ()+ ": "+ e  );
                    }
                    ...
                    //                      //chiamo notify e  invio della notifica
                                          ...
                                          System.out.println ( methodName+" - errore nell'invio delle notifiche tipizzate per ente  "+ente.getIdEnte()+": "+ e  );
                                      }
                                  }
		...
		try ( CloseableHttpClient httpClient = HttpClients.createDefault ();
			...
			context.info ( methodName, "Ottengo la risposta con stato: " + response.getStatusLine ().getStatusCode () );
			 System.out.println ( methodName+" - Ottengo la risposta con stato:  "+response.getStatusLine ().getStatusCode () );
		    	...
		    	System.out.println ( methodName+" - Errore nella chiamata "+e );
...
//	     postRequest.addHeader ( HttpHeaders.AUTHORIZATION, token );
    ...
    public String buildJsonInvio(List<Message> model)  {
		final String methodName = "buildJsonInvio";
		 ...
		 System.out.println ( methodName+" - avvio" );
		...
		try {
			...
			context.error(methodName, "errore nella creazione del model json ", e);
			 System.out.println ( methodName+" - errore nella creazione del model json  "+e );
			return "";
		}
