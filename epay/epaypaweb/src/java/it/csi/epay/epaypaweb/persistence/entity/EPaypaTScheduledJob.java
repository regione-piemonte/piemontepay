...
/**
 * The persistent class for the epaypa_t_scheduled_job database table.
 *
 */
@Entity
@Table(name="epaypa_t_scheduled_job")
@NamedQueries({
		@NamedQuery(
				name = "EPaypaTScheduledJob.findAllJobs",
				query= "SELECT e "
						+ "FROM EPaypaTScheduledJob e "),
		@NamedQuery(
				name = "EPaypaTScheduledJob.findOneActiveJob",
				query= "SELECT e "
						+ "FROM EPaypaTScheduledJob e "
						+ "WHERE e.active = true "
						+ "AND e.jobType = :jobType "),
		@NamedQuery(
				name = "EPaypaTScheduledJob.findNextActiveJob",
				query= "SELECT e "
						+ "FROM EPaypaTScheduledJob e "
						+ "WHERE e.active = true "
						+ "AND (EXTRACT(epoch FROM (current_timestamp - e.ultimaEsecuzione)) > e.interval OR e.ultimaEsecuzione IS NULL) ")
})
public class EPaypaTScheduledJob implements Serializable {
