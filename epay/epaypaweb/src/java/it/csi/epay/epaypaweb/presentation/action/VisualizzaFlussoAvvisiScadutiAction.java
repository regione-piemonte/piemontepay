...
@Results({
//@formatter:off
	@Result(name = INPUT, location = "pages/visualizza-flusso-avvisiscaduti.jsp"),
	@Result(name = "json-table-data", type = "json", params = { "root", "dataTableResponse", "noCache", "true" })
//@formatter:on
})
public class VisualizzaFlussoAvvisiScadutiAction extends VisualizzaFlussoBaseAction {
	static private final long serialVersionUID = 1L;
	static private final String CLASSNAME = VisualizzaFlussoAvvisiScadutiAction.class.getSimpleName();

	private DataTableResponse<AvvisoScadutoDto> dataTableResponse;

	@EJB
	private GestioneFlussiBusiness gestioneFlussiBusiness;

	@Action("entry-visualizza-flusso-avvisiscaduti")
	@Authorization(cdu = "DET_AS")
	@SkipValidation
	public String entryVisualizzaFlussoAvvisiScaduti() {
		String methodName = "entryVisualizzaFlussoAvvisiScaduti";
	...
	@Action("visualizza-flusso-avvisiscaduti-json")
	@Authorization(cdu = "DET_AS")
	@SkipValidation
	public String visualizzaFlussoAvvisiScadutiJSON() throws Exception {
		String methodName = "visualizzaFlussoAvvisiScadutiJSON";
		
		log.info ( CLASSNAME + " " + methodName + " - START" );

		List<CriterioOrdinamentoDto<ColumnNameAvvisoScadutoEnum>> sorting = null;
		if (sortingCol != null && sortingDir != null) {
			sorting = new ArrayList<CriterioOrdinamentoDto<ColumnNameAvvisoScadutoEnum>>();
			CriterioOrdinamentoDto<ColumnNameAvvisoScadutoEnum> sortingItem = new CriterioOrdinamentoDto<ColumnNameAvvisoScadutoEnum>();

			if ("IUV".equals(sortingCol))
				sortingItem.setColumnNameEnum(ColumnNameAvvisoScadutoEnum.IUV);
			else if ("importo".equals(sortingCol))
				sortingItem.setColumnNameEnum(ColumnNameAvvisoScadutoEnum.IMPORTO);
			else if ("dataScadenza".equals(sortingCol))
				sortingItem.setColumnNameEnum(ColumnNameAvvisoScadutoEnum.DATA_SCADENZA);
		...
		try {
			TotalSizeAndLightListDto<AvvisoScadutoDto> totalSizeAndLightList = gestioneFlussiBusiness.getTotalSizeAndAvvisoScadutoList(idFlusso, sorting, pag);

			Long count = totalSizeAndLightList.getTotalSize();

			dataTableResponse = new DataTableResponse<AvvisoScadutoDto>();
	...
	public DataTableResponse<AvvisoScadutoDto> getDataTableResponse() {
		return dataTableResponse;
	}

	public void setDataTableResponse(DataTableResponse<AvvisoScadutoDto> dataTableResponse) {
