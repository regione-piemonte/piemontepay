...
import it.csi.epay.epaypaweb.dto.mapper.TemplateMapperNotificaPagamento;
...
@Action("scarica-flusso-pagamenti")
@Results({
//@formatter:off
...
public class DownloadFlussoPagamentiAction extends DownloadFlussoAction {
	static private final long serialVersionUID = 1L;
	static private final String CLASSNAME = DownloadFlussoPagamentiAction.class.getSimpleName();

	@EJB
	private TemplateBusiness templateBusiness;

	@Override
	@Authorization(cdu = "DLD_ESP")
	public String execute() {
		String methodName = "execute";
		

		FlussoCompletoDto<NotificaPagamentoDto> flusso = null;
		try {
			log.info ( CLASSNAME + " " + methodName + " - START" );

			FlussoNotifichePagamentoRequestDto flussoNotifichePagamentoRequestDto = new FlussoNotifichePagamentoRequestDto(getIpAddress(),getIdUtente(),getCodiceFiscaleUtente(),getCodiceApplicazione(),idFlusso);
			flusso = gestioneFlussiBusiness.getFlussoNotifichePagamento(flussoNotifichePagamentoRequestDto,true);
			//flusso = gestioneFlussiBusiness.getFlussoNotifichePagamento(idFlusso);

			TemplateDto template = templateBusiness.getTemplate(flusso.getFlusso().getIdEnte(), flusso.getFlusso().getTipoFlusso().getId());

			if (template != null) {
				byte[] data = templateBusiness.generate(flusso, template, new TemplateMapperNotificaPagamento(), tipoFormato).getData();
