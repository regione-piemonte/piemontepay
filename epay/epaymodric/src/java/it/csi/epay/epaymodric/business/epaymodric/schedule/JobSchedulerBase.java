...
import it.csi.epay.epaymodric.business.epaymodric.schedule.plan.SchedulePlan;
import it.csi.epay.epaymodric.business.epaymodric.schedule.plan.SchedulePlanConfig;
import it.csi.epay.epaymodric.business.epaymodric.schedule.plan.TimeInterval;
...
public class JobSchedulerBase {
	...
	protected SchedulePlanConfig buildSchedulePlanConfig(String cronExpression) throws IllegalArgumentException {
		...
		for (int i = 0; i < ranges.length; i++) {
			...
			if (ranges[i].charAt(0) == '*') {
				...
				int openParenthesis = ranges[i].indexOf('(');
				if (openParenthesis == -1)
					openParenthesis = ranges[i].length();
				else if (!ranges[i].endsWith(")")) {
					// manca la parentesi chiusa
					throw new IllegalArgumentException("Missing right parenthesis " + ranges[i]);
				}

				String daysExpression = ranges[i].substring(0, openParenthesis);
				...
				if (minusIndex > -1 && commaIndex > -1) // ci sono sia meno che virgole
					throw new IllegalArgumentException("Cannot use minus and commas in same expression " + ranges[i]);
			...
			int openParenthesis = ranges[i].indexOf('(');
			if (openParenthesis > -1) {
				String timeExpression = ranges[i].substring(openParenthesis + 1, ranges[i].length() - 1);
				...
				for (int t = 0; t < tokens.length; t++) {
					...
					if (hourExpression.length() == 1) {
						...
						if (hourExpression.length() == 11 && hourExpression.charAt(2) == ':' && hourExpression.charAt(5) == '-' && hourExpression.charAt(8) == ':') {
							try {
	...
	private int getDOWArrayIndex(Integer calendarDOW) {
		for (int i = 0; i < daysOfWeekArray.length; i++) {
			if (daysOfWeekArray[i] == calendarDOW.intValue()) {
				return i;
			}
		}
		return -1;
