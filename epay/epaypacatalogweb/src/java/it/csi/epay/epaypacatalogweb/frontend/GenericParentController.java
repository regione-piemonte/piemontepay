...
import it.csi.epay.epaypacatalogweb.integration.stubs.epaypacatalogsrv.MessageDto;
...
public class GenericParentController {
    ...
    public String concatenaMessaggi ( List<MessageDto> messaggi ) {
        String concatenati = "";

        for ( MessageDto messaggio: messaggi ) {
            concatenati += messaggio.getMessaggio () + "; \n";
        }

        if ( messaggi.size () > 0 ) {
            concatenati = concatenati.substring ( 0, concatenati.length () - 3 );
        }
